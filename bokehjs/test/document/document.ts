...
class ModelWithConstructTimeChanges extends Model {
  ...
  initialize(): void {
    super.initialize()
    this.foo = 4
    this.child = new ModelWithConstructTimeChanges()
  }
...
namespace ComplicatedModelWithConstructTimeChanges {
  export type Attrs = p.AttrsOf<Props>
  export type Props = Model.Props & {
    list_prop: p.Property<Model[]>
    dict_prop: p.Property<{[key: string]: Model}>
    obj_prop: p.Property<Model>
...
describe("Document", () => {
  ...
  it("sets proper document on models added during construction", () => {
    ...
    const root1_copy = copy.get_model_by_id(root1.id)
  ...
  it("computes complicated patch for models added during construction", () => {
    ...
    for (const key in serialized_values) {
      const value = serialized_values[key] // XXX: own
